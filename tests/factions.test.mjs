...
test('existing and blank factions open full editable profiles, save and reopen',async()=>{
 ...
 const page=await req('/factions/sample-ember/');assert.equal(page.status,200);const html=await page.text();for(const key of factionFields)assert.ok(html.includes('name="'+key+'"'));assert.ok(html.includes('/characters/claude/'));
 ...
 const doc={...blank,name:'The River Guard',history:'First event\nSecond event',type:'Order',leaderId:'claude',allies:'House of Ember'};response=await req('/api/factions/'+id,'PUT',doc);assert.equal(response.status,200);const saved=await response.json();assert.equal(saved.version,1);
 const reopened=await (await req('/api/factions/'+id)).json();assert.equal(reopened.history,doc.history);assert.equal(reopened.name,doc.name);

...
const record=()=>({...blankLore('note'),id:'research-note',version:4,name:'Research',body:'Plain text [1]\n<em>Literal markup</em>',originDate:'The first winter'});
...
test('note serialization preserves text and reference identity without leaking display projections or metadata',()=>{
 ...
 assert.equal(payload.version,4);assert.deepEqual(payload.collections,['notes']);
 assert.equal('id' in payload,false);assert.equal('updatedAt' in payload,false);assert.equal('key' in payload.connections[0],false);
 assert.deepEqual(Object.keys(payload).sort(),[...loreTemplates.note.fields,'type','version','hiddenFields','collections','profileRatings','pinned','featured','connections'].sort());
});

...
test('countries, cities, and landmarks retain a complete hierarchy after creation and reload',async()=>{
 ...
 for(const record of [country,city,landmark]){const response=await request('POST',record);assert.equal(response.status,201);assert.deepEqual(await response.json(),record.type==='landmark'?{...record,version:0,schemaVersion:1}:record);}

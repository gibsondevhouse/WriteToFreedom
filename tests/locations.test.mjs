...
test('countries, cities, and landmarks retain a complete hierarchy after creation and reload',async()=>{
 ...
 for(const record of [country,city,landmark]){const response=await request('POST',record);assert.equal(response.status,201);assert.deepEqual(await response.json(),record);}
...
test('location mutations require identity, same origin, a valid type, and a nonblank name',async()=>{
 ...
 assert.equal((await request('PUT',record)).status,405);
});

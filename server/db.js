...
export function repository(binding) {
 ...
 return {
  async listStoryArcs(owner) {return (await binding.prepare('SELECT * FROM story_arcs WHERE owner_id = ? ORDER BY updated_at DESC, id').bind(owner).all()).results.map(row=>({...JSON.parse(row.document),id:row.id,version:row.version,createdAt:row.created_at,updatedAt:row.updated_at}));},
  async getStoryArc(owner,id) {const row=await binding.prepare('SELECT * FROM story_arcs WHERE owner_id = ? AND id = ?').bind(owner,id).first();return row?{...JSON.parse(row.document),id:row.id,version:row.version,createdAt:row.created_at,updatedAt:row.updated_at}:null;},
  ...
  async saveStoryArc(owner,id,version,document) {const result=await binding.prepare('UPDATE story_arcs SET document = ?, version = version + 1, updated_at = ? WHERE owner_id = ? AND id = ? AND version = ?').bind(JSON.stringify(document),new Date().toISOString(),owner,id,version).run();return result.meta.changes?this.getStoryArc(owner,id):null;},

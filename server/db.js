...
export function repository(binding) {
 ...
 return {
  ...
  async listCityProfiles(owner) {return (await binding.prepare('SELECT * FROM city_profiles WHERE owner_id = ?').bind(owner).all()).results.map(row=>({...decodeStoredDocument(row),id:row.location_id,version:row.version,schemaVersion:row.schema_version??supportedSchemaVersion}));},
  async saveCity(owner,id,version,document) {
   ...
   const [result]=await binding.batch([write,rename]);return result.meta.changes?{...document,id,version:version+1,schemaVersion:supportedSchemaVersion}:null;
  },
  async listCountryProfiles(owner) {return (await binding.prepare('SELECT * FROM country_profiles WHERE owner_id = ?').bind(owner).all()).results.map(row=>({...decodeStoredDocument(row),id:row.location_id,version:row.version,schemaVersion:row.schema_version??supportedSchemaVersion}));},
  ...
  async listLocationDetails(owner) {return (await binding.prepare('SELECT * FROM location_details WHERE owner_id = ?').bind(owner).all()).results.map(row=>({...decodeStoredDocument(row),id:row.location_id,version:row.version,schemaVersion:row.schema_version??supportedSchemaVersion}));},
  async saveLocationDetails(owner,id,version,document) {
   const encoded=JSON.stringify(document);
   const write=version===0?binding.prepare('INSERT INTO location_details (owner_id, location_id, document, version) VALUES (?, ?, ?, 1) ON CONFLICT(owner_id, location_id) DO NOTHING').bind(owner,id,encoded):binding.prepare('UPDATE location_details SET document = ?, version = version + 1 WHERE owner_id = ? AND location_id = ? AND version = ? AND schema_version = 1').bind(encoded,owner,id,version);
  ...
  async saveFaction(owner,id,version,document) {
   ...
   const [result]=await binding.batch([statement,rename]);return result.meta.changes?{...document,id,version:version+1,schemaVersion:supportedSchemaVersion}:null;
  },

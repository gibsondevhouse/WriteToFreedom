...
export async function timelineRoute(request,env){
 ...
 try{
  ...
  return json(collectTimeline({...locationProfileGroups(locations),lore,storyArc:storyArcs,character:characterCast(characters),faction:factions,country:locations.filter(l=>l.type==='country').map(l=>({...defaultCountry(l),...countryProfiles.get(l.id)})),city:locations.filter(l=>l.type==='city').map(l=>({...defaultCity(l),...cityProfiles.get(l.id)}))}));

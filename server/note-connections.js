...
import {typeLabels} from '../public/locations/data.js';
import {escape,renderSection} from './profile-components.js';
export function noteTargets(cast,factions,locations){
 return [
 ...
 ...locations.map(l=>({kind:'location',id:l.id,label:l.name,group:'Places',detail:typeLabels[l.type],href:['country','city'].includes(l.type)?'/locations/'+(l.type==='country'?'countries':'cities')+'/'+encodeURIComponent(l.id)+'/':'/locations/#location-'+encodeURIComponent(l.id)})),
...
export function connectedNotes(cast,target){
 const key=referenceKey(target);
 return cast.flatMap(c=>(c.notes||[]).filter(n=>(n.links||[]).some(l=>referenceKey(l)===key)).map(n=>({source:c.name||'Untitled character',title:n.title||'',type:n.type||'detail',text:n.text,href:'/characters/'+encodeURIComponent(c.id)+'/#note-'+n.id})));
}
export function renderConnectedNotes(cast,target,record){
 const notes=connectedNotes(cast,target);if(!notes.length)return '';

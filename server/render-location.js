...
import {escape,createFieldRenderer,renderInfoGroup,renderProfileName,renderProfilePage,renderSection,renderRatingsHost} from './profile-components.js';
...
export function renderLocation(record,locations,cast,lore=[]){
 ...
 const ratingGroups=ratingGroupsFor('location',record.type);
 ...
 const content=template.sections.filter(s=>!['identity','symbols'].includes(s.id)).map(s=>renderSection(s,(s.id==='places'?places+'<p class="section-note"><a href="/locations/">Manage locations</a></p>':s.fields.map(field).join(''))+renderRatingsHost(ratingGroups,s.id),record,{fullWidth:s.id==='places'})).join('');
 ...
 return renderProfilePage({record,type:record.type,collection:'Locations',collectionUrl:'/locations/',infobox,
  ...
  styles:['/characters/profile-notes.css','/characters/attribute-controls.css','/profiles/ratings.css','/locations/countries/profile.css','/locations/profile.css'],boxClass:'country-infobox location-infobox'});
}

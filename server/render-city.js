...
export function renderCity(city,locations,cast,lore=[]){
 const ratingGroups=ratingGroupsFor('location','city');
 ...
 (s.id==='places'?places+'<p class="section-note"><a href="/locations/">Manage locations</a></p>':'')+s.fields.map(field).join('')+renderRatingsHost(ratingGroups,s.id),city,{fullWidth:s.id==='places'})).join('');

...
function normalize(record){return {...blankStoryArc(),...record,pacing:{...blankStoryArc().pacing,...(record.pacing||{})},keyEntities:record.keyEntities||[],connectedArcIds:record.connectedArcIds||[],keyScenes:record.keyScenes||[],hiddenFields:record.hiddenFields||[]};}
function validate(input,current,targets,arcs){
 ...
 document.name=document.name.trim().replace(/\s+/g,' ');if(!document.name)throw new Error('Enter a story arc title.');
 ...
 const scenes=Object.hasOwn(input,'keyScenes')?input.keyScenes:current.keyScenes;if(!Array.isArray(scenes)||scenes.length>60)throw new Error('Keep at most 60 key scenes.');document.keyScenes=scenes.map(scene=>{if(!scene||typeof scene!=='object'||Array.isArray(scene))throw new Error('Check the key scene details.');const clean={};for(const key of ['title','chapter','beat','summary']){if(typeof scene[key]!=='string'||scene[key].length>(key==='title'?160:10000))throw new Error('Check the key scene details.');clean[key]=scene[key].trim();}if(!clean.title)throw new Error('Give every key scene a title.');if(clean.beat&&!arcBeats.some(beat=>beat.id===clean.beat))throw new Error('Choose a valid narrative beat for every key scene.');return clean;});
 return document;
}
export async function storyArcRoute(request,env){
 ...
 try{
  ...
  if(!id){if(!idPattern.test(input.id))return json({error:'Choose a valid story arc ID.'},400);const existing=arcs.find(arc=>arc.id===input.id);if(existing)return json(existing,201);const created=await db.createStoryArc(owner,input.id,blankStoryArc());return created?json(normalize(created),201):json({error:'This story arc could not be created.'},409);}
  if(!Number.isInteger(input.version)||input.version<1)return json({error:'Reload this story arc before saving.'},400);if(input.version!==current.version)return json({error:'This story arc changed in another tab. Copy your unsaved text, then reload before saving.'},409);

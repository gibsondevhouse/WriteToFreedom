...
export function renderCountry(country,locations,cast){
 ...
 const field=createFieldRenderer(country,{required:['name'],options:(key,type)=>{const list=type==='city'?cities:type==='character'?cast:null;return list?list.map(c=>[c.id,c.name||'Untitled character']):null;},links:{leaderId:'/characters/'}});
 ...
 const infobox=renderProfileName(country,'country',{official:true})+
  ...
  renderInfoGroup('Geography & population','identity-geography',fields(['capitalId','largestCityId','area','population','populationDate']))+

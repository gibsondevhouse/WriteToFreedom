...
export function renderCountry(country,locations,cast,lore=[]){
 const ratingGroups=ratingGroupsFor('location','country');
 ...
 (s.id==='places'?places+'<p class="section-note"><a href="/locations/">Manage locations</a></p>':'')+s.fields.map(field).join('')+renderRatingsHost(ratingGroups,s.id),country,{fullWidth:s.id==='places'})).join('');

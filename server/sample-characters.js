...
export function sampleCharacter(id) {
 ...
 return {...blankCharacter(),...profile,id,version:0,schemaVersion:1,name:seed.name,firstName:seed.name,title:seed.title,roles:seed.roles.join(' · '),affiliation:seed.affiliation,factionId:seedFactions.find(f=>f.name===seed.affiliation)?.id||'',storyRole:seed.attributes['Story role'],summary:seed.summary,biography:seed.biography,strength:seed.attributes.Strength,flaw:seed.attributes.Flaw,desire:seed.attributes.Desire,fear:seed.attributes.Fear,tendencies:seed.tendencies.join('\n'),questions:profile.questions.join('\n'),relationships:seed.relationships.map(r=>({targetId:r.id,type:r.type,description:r.text}))};
}

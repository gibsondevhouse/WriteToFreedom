...
const directory=initDirectoryShell({
 async load(options){
  ...
  return data.characters.map(character=>({...character,href:novelContextHref('/characters/'+encodeURIComponent(character.id)+'/'),provider:characters.find(sample=>sample.id===character.id)?.provider||''}));
 },

...
function characterCard(record){const link=cardLink(record,'portrait-card'),body=el('div','card-copy'),role=record.roles.join(' · ')||record.storyRole||'Character';link.title=record.name+' — '+role;body.append(el('h3','',record.name),el('p','meta',role));link.append(cover(record),body);return link;}

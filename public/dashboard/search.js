...
export function searchCatalog(data, query) {
  ...
  return [...data.characters, ...data.factions, ...data.locations,...(data.lore||[])].filter(record => {
    const text = [record.name, record.label, record.summary, record.title,
      ...
      record.areaType,record.tags,record.searchText,...(record.collections||[]), ...(record.roles || [])].filter(Boolean).join(' ').toLocaleLowerCase();

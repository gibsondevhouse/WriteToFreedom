 ...
 * unload warning. The returned version setter lets a page coordinate a separate
 * optimistic mutation that advances this same document. Character saving is separate.
 * @param {object} config fieldNames, endpoint, type, optional imageFields/validImageUrl, readExtra() and onSaved(data).
 * @returns {{setVersion: function(number): void, readonly dirty: boolean}}
 */
export function initProfileEditor({fieldNames,endpoint,type,nameField='name',imageFields=[],validImageUrl=()=>false,onSaved=()=>{},readExtra=()=>({})}){
 ...
 function update(){
  const name=form.elements.namedItem(nameField).value.trim()||'Untitled '+type;
 ...
 const controls=initProfileControls(form,markDirty,{nameField,choiceSelections:initial.choiceSelections});
 ...
 return {setVersion(next){if(Number.isSafeInteger(next)&&next>0){version=next;form.dataset.version=String(next);}},get dirty(){return dirty;}};
}

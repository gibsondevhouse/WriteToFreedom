...
const directory=initDirectoryShell({
 ...
 async load(options){
  ...
  try{const data=await fetchDirectory('/api/locations',options);contextLocations=data.contextLocations||data.locations;return data.locations;}
 ...
 renderItem(location,state,index){
  const records=directory.records,parents=ancestors(location,contextLocations),children=records.filter(r=>r.parentId===location.id);
  ...
  const path=parents.map(parent=>parent.name).join(' › '),record={...location,href:novelContextHref(href(location)),image:location.image||location.imageUrl||location.skylineUrl||location.flagUrl||''};
...
function populateParent(selected=''){
 ...
 const options=parentChoices(type.value,parentCatalog,editing?.id).sort((a,b)=>a.name.localeCompare(b.name));
 options.forEach(r=>{const path=ancestors(r,parentCatalog).map(p=>p.name).join(' › ');parent.append(new Option(r.name+(path?' — '+path:''),r.id));});parent.value=selected;
...
form.addEventListener('submit',async event=>{event.preventDefault();if(saving)return;if(!name.value.trim()){name.setCustomValidity('Enter a location name.');name.reportValidity();return;}if(!form.reportValidity())return;
 ...
 try{
  ...
  location.assign(novelContextHref(href(saved)));return;
